...
tabs, screenshot, read titles, take an accessibility snapshot with stable element
refs, and drive the page with synthetic click/type/scroll input. Core spawns
...
- **Sidecar:** `browser` on `:7993`, `command: "ryu-browser"`,
  ...
  `navigate`/`screenshot`/`eval`/`snapshot`/`title`, and the flat input routes
  `/click`, `/type`, `/scroll`.
- **Provides:** capability `browser.control` (v1.0.0) → sidecar `browser`, route
  `/`, grant `browser:control`, binding all seven canonical verbs —
  `browser__tabs`/`navigate`/`screenshot`/`snapshot`/`click`/`type`/`scroll`.
  (`browser__eval` is deliberately not a verb; see `docs/swappable-layers-design.md`
  §2.4.)
