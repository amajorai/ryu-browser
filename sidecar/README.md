...
The desktop Browser panel sends the resolved Desktop appearance snapshot through
the authenticated `PUT /theme` route. The sidecar applies its colors, fonts,
locale, and display time zone live to the browser chrome; it never injects these
tokens into websites opened in tabs.
...
| `PUT` | `/theme` `{tokens}` | Apply the bounded Desktop appearance snapshot to the browser chrome |
