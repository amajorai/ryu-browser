...
import { formatBytes, formatRelativeTime } from "../main/browser-state.ts";
...
void bridge
	?.getTheme()
	.then(applyThemeTokens)
	.catch(() => undefined);
const removeThemeListener = bridge?.onTheme((tokens) => {
	applyThemeTokens(tokens);
	render();
});
window.addEventListener("beforeunload", () => removeThemeListener?.(), {
	once: true,
});
...
function renderSettings(snapshot: BrowserState): void {
	const page = renderPageShell(
		"Browser",
		"Manage the built-in browser. Browser extensions can be set up in device control settings",
		"browser"
	);
...
function renderHistoryRow(
	...
	copy.append(
		...
		el("div", "history-time", formatRelativeTime(entry.visitedAt))
	);
