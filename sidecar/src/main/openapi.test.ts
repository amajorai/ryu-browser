...
/** The sub-paths the manifest's hand-written `chromium__*` runnables already serve. */
...
describe("openapi document contract", () => {
	...
	// already ships as a hand-written `chromium__*` runnable whose description carries
	...
	it("documents nothing the hand-written runnables already cover", () => {
		...
		for (const route of Object.keys(OPENAPI_DOCUMENT.paths)) {
			...
			expect(
				covered.has(route),
				`${route} duplicates an existing chromium__* runnable`
			).toBe(false);
		}
	});
