...
// manifest ships 11 `runnables` (`chromium__list_tabs`, `chromium__open_tab`,
// `chromium__snapshot_tab`, `chromium__click`, …) covering every tab, input and
// capture route this sidecar serves, and they are strictly better than anything a
// derivation could produce:
//
//   * their descriptions are written against the canonical `browser__*` verbs the
...
// then has to choose between `chromium__click` and `ryu_ext__ryu_browser__post_click`
...
export const OPENAPI_DOCUMENT = {
	...
	info: {
		...
		version: "1.0.0",
		description:
			"Control surface of the local real-Chromium browser sidecar. Tab management, page capture, accessibility snapshots and synthetic click/type/scroll input are exposed to agents as the app's own `chromium__*` tools rather than through this document, so only the service and liveness routes are described here.",
	},
	...
	paths: {
		"/": {
			get: {
				...
				description:
					"Identity and version of the running browser control sidecar, plus the capability it backs (`browser.control`). Use it to confirm which browser build is answering; use the `chromium__list_tabs` tool to see what is actually open in it.",
