...
import {
	agentLaneForCaller,
	bearerOk,
	blockedNavigationAddress,
	bodyWithinControlLimit,
	handleRequest,
	isJsonContentType,
...
describe("control request limits", () => {
	it("rejects negative, fractional, unsafe, and oversized lengths", () => {
		expect(bodyWithinControlLimit(0)).toBe(true);
		expect(bodyWithinControlLimit(1024 * 1024)).toBe(true);
		expect(bodyWithinControlLimit(1024 * 1024 + 1)).toBe(false);
		expect(bodyWithinControlLimit(-1)).toBe(false);
		expect(bodyWithinControlLimit(1.5)).toBe(false);
		expect(bodyWithinControlLimit(Number.MAX_SAFE_INTEGER + 1)).toBe(false);
	});
});

describe("managed Bot lane binding", () => {
	it("accepts only Core's stamped lane and rejects bare query selectors", () => {
		expect(agentLaneForCaller("bot-b", "bot-a")).toBe("bot-a");
		expect(agentLaneForCaller("bot-b", undefined, "user-a")).toBeUndefined();
		expect(agentLaneForCaller("bot-b", "  ", undefined)).toBeUndefined();
	});
});
...
describe("navigation destination guard", () => {
	it("rejects literal private, mapped, and metadata destinations", async () => {
		for (const address of [
			...
			expect(blockedNavigationAddress(address)).toBe(true);
			const host = address.includes(":") ? `[${address}]` : address;
			expect(() => safeNavigationUrl(`https://${host}/`)).toThrow(
				"private or local"
			);
		}
		expect(() =>
			safeNavigationUrl("https://metadata.google.internal/")
		).toThrow("private or local");
	});

	it("preserves public literal navigation after screening", async () => {
		await expect(safeNavigationUrlResolved("https://1.1.1.1/")).resolves.toBe(
			"https://1.1.1.1/"
		);
	});
});
...
describe("handleRequest auth", () => {
	...
	it("accepts the authenticated Desktop theme sync while control is disabled", async () => {
