...
/** JSON-safe metadata for one tool registered by the active web page. */
...
export interface BrowserWebMCPExecution {
	result: unknown;
...
export interface TabManager {
	...
	/** Execute one page-registered WebMCP tool with a JSON object input. */
	webmcpExecute(
		id: string,
		name: string,
		input: Record<string, unknown>
	): Promise<BrowserWebMCPExecution | null>;
	/** List structured tools registered by the active document in tab `id`. */
	webmcpTools(id: string): Promise<BrowserWebMCPTool[] | null>;
}
